...
import com.xss6.sca.hack.hacks.chat.ToggleMessages;
...
public class ClientMessage implements Globals {
    ...
    private static final String opener() {
        if (mc.player.getName().equalsIgnoreCase("XSS6"))
            return ChatFormatting.DARK_BLUE + "[" + ChatFormatting.BLUE + "SCA" + ChatFormatting.DARK_BLUE + "] " + ChatFormatting.RESET;
        return ChatFormatting.GOLD + ScaMod.NAME + ChatFormatting.WHITE + " : " + ChatFormatting.RESET;
    }

    public static void sendToggleMessage(Hack hack, boolean enabled) {
        if(mc.world != null && mc.player != null) {
            if(ScaMod.HACKS.ishackEnabled("Toggle msgs")) {
                ...
                if (hack.getName().equalsIgnoreCase("hud")) {
    ...
    public static void sendOverwriteClientMessage(String message) {
        if (mc.player != null) {
            final ITextComponent itc = new TextComponentString(opener() + message).setStyle(new Style().setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TextComponentString("XSS6"))));
