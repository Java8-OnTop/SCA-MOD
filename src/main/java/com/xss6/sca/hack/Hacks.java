...
import com.xss6.sca.event.events.Render2DEvent;
...
public class Hacks implements Globals{
	...
	public Hacks() {
		...
		try {
			ArrayList<Class<?>> modClasses = ReflectionUtil.getClassesForPackage("com.xss6.sca.hack.hacks");
	...
	public void onUnload() {
		...
		this.hacks.forEach(ScaMod.EVENT_PROCESSOR::removeEventListener);
	...
	public final void onKeyDown(int key) {
		if (key <= 0 || mc.currentScreen instanceof ScaGuiNew) {
			return;
		}
		for (Hack hack : this.hacks) {
	...
	public final List<Hack> getSortedHacks(boolean reverse, boolean customFont) {
		if (customFont) {
			if (reverse) {
				List<Hack> list = this.getEnabledHacks().stream().sorted(Comparator.comparing(hack -> ScaMod.GUI_FONT_MANAGER.getTextWidth(Hack.getFullArrayString()))).collect(Collectors.toList());
				Collections.reverse(list);
				return list;
			} else {
				return this.getEnabledHacks().stream().sorted(Comparator.comparing(hack -> ScaMod.GUI_FONT_MANAGER.getTextWidth(Hack.getFullArrayString()))).collect(Collectors.toList());
			}
		} else {
			if (reverse) {
				List<Hack> list = this.getEnabledHacks().stream().sorted(Comparator.comparing(hack -> mc.fontRenderer.getStringWidth(Hack.getFullArrayString()))).collect(Collectors.toList());
				Collections.reverse(list);
				return list;
			} else {
				return this.getEnabledHacks().stream().sorted(Comparator.comparing(hack -> mc.fontRenderer.getStringWidth(Hack.getFullArrayString()))).collect(Collectors.toList());
			}
		}
	}
}
